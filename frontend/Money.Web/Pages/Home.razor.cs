...
public partial class Home
{
    ...
    [
        new("1.3.17", new(2026, 5, 4), [
            new("На страницах операций, шаблонов, регулярных операций и машинок добавлены индикатор загрузки и подсказка с иконкой, когда список пуст.", ChangeType.UiUx),
            new("Карточки операций, долгов и плашка с ошибкой теперь используют цвета темы и корректно перерисовываются при переключении светлой и тёмной.", ChangeType.UiUx),
            new("Кнопки «Добавить» на страницах долгов, шаблонов и регулярных операций сделаны заметнее (залитые вместо контурных).", ChangeType.UiUx),
            new("В диалогах быстрой и регулярной операции форму теперь можно отправить нажатием Enter.", ChangeType.UiUx),
            new("При добавлении машинки фокус сразу ставится на поле названия.", ChangeType.UiUx),
            new("В диалоге события машинки выбор типа стал доступен с клавиатуры (Tab, Enter, Пробел) и для скринридеров.", ChangeType.Improvement),
            new("Всплывающее окно оплаты долга больше не закрывает иконку при открытии: раскрывается вниз.", ChangeType.BugFix),
            new("На узких экранах кнопки выбора диапазона в фильтре операций сокращаются до одной буквы (Д/Н/М/Г), полное название остаётся во всплывающей подсказке; на широких показываются как раньше. Нижний ряд кнопок переносится на узких экранах.", ChangeType.UiUx),
            new("Иконки кнопок и адорнментов переведены на единый стиль (Rounded) для визуальной согласованности.", ChangeType.UiUx),
            new("В списке операций длинная сумма больше не наезжает на колонку категории на узких экранах: ширина колонки ограничена, лишний текст обрезается с троеточием.", ChangeType.BugFix),
        ]),
