...
[Route("[controller]")]
public class OperationsController(OperationsService operationsService, PlacesService placesService) : ControllerBase
{
    /// <summary>
    ...
    public async Task<IActionResult> Get([FromQuery] OperationFilterDto filter, CancellationToken cancellationToken)
    {
        ...
        var models = await operationsService.GetAsync(businessFilter, cancellationToken);
        return Ok(models.Select(OperationDto.FromBusinessModel));
    }
    ...
    {
        var model = await operationsService.GetByIdAsync(id, cancellationToken);
        return Ok(OperationDto.FromBusinessModel(model));
    }
    ...
    public async Task<IActionResult> Create([FromBody] SaveRequest request, CancellationToken cancellationToken)
    {
        ...
        var id = await operationsService.CreateAsync(model, cancellationToken);
    ...
    public async Task<IActionResult> Update(int id, [FromBody] SaveRequest request, CancellationToken cancellationToken)
    {
        var model = request.ToBusinessModel();
        model.Id = id;
        await operationsService.UpdateAsync(model, cancellationToken);
        return NoContent();
    }
    ...
    {
        var models = await operationsService.UpdateBatchAsync(request.OperationIds, request.CategoryId, cancellationToken);
        return Ok(models.Select(OperationDto.FromBusinessModel));
    }
    ...
    {
        await operationsService.DeleteAsync(id, cancellationToken);
        return NoContent();
    }
    ...
    {
        await operationsService.RestoreAsync(id, cancellationToken);
        return NoContent();
    }
    ...
    {
        IEnumerable<Place> models = await placesService.GetPlacesAsync(offset, count, name, cancellationToken);
    ...
    {
        var businessFilter = filter.ToBusinessModel();
        var operations = (await operationsService.GetAsync(businessFilter, cancellationToken)).ToArray();
        var categories = (await categoriesService.GetAsync(cancellationToken: cancellationToken)).ToArray();
